...
const PercentageGrid = ({ shadedSquares, totalSquares, description, onAnswer, onRequestHelp }: { 
  ...
  const handleSubmit = () => {
    ...
    if (Math.abs(answer - correctAnswer) < 0.1) {
      ...
      const helpContext = `The student is trying to find what percentage is illustrated when ${shadedSquares} out of ${totalSquares} squares are shaded. They have attempted this ${attempts + 1} times unsuccessfully.`;
  ...
  return (
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      />
      <CardHeader>
        <CardTitle className="text-lg text-white">Grid Model Practice</CardTitle>
        <CardDescription className="text-gray-300">
          {description}. What percentage is illustrated?
        </CardDescription>
      </CardHeader>
      <CardContent>
...
const StripModel = ({ percentage, description, onAnswer, onRequestHelp }: { 
  ...
  return (
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d]">
      <CardHeader>
        <CardTitle className="text-lg text-white">Strip Model Practice</CardTitle>
        <CardDescription className="text-gray-300">
...
// Interactive decimal-percent conversion component
const DecimalPercentConversion = ({ 
  ...
  const { 
    ...
    animationType, 
    handleAnimationComplete 
  } = useFeedbackLogic(onRequestHelp, 1);
  ...
  const handleSubmit = () => {
    ...
    if (Math.abs(answer - correctAnswer) < 0.001) {
      handleCorrectAnswer(`Correct! ${type === 'decimal' ? `${value} = ${correctAnswer}%` : `${value}% = ${correctAnswer}`}`);
      onAnswer(answer);
    } else {
      const question = `How do I convert ${type === 'decimal' ? 'decimal to percent' : 'percent to decimal'} when ${description}?`;
      const context = `The student is trying to convert ${type === 'decimal' ? `${value} to a percent` : `${value}% to a decimal`}. The correct answer is ${correctAnswer}. They have attempted this ${attempts + 1} times unsuccessfully.`;
      const customMessage = `Not quite. Remember: ${type === 'decimal' ? 'multiply by 100 to get percent' : 'divide by 100 to get decimal'}. Try again! (Attempt ${attempts + 1} of 3)`;
    ...
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardHeader>
        <CardTitle className="text-lg text-white">
          {type === 'decimal' ? 'Decimal to Percent' : 'Percent to Decimal'} Conversion
        </CardTitle>
        <CardDescription className="text-gray-300">
          {description}
        </CardDescription>
    ...
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="bg-gray-100 p-6 rounded-lg border-2 border-gray-300">
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-800 mb-2">{decimal}</div>
              <div className="text-lg text-gray-600">Convert to fraction</div>
            </div>
          </div>
    ...
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="bg-gray-100 p-6 rounded-lg border-2 border-gray-300">
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-800 mb-2">{fraction}</div>
              <div className="text-lg text-gray-600">Convert to decimal</div>
            </div>
          </div>
    ...
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="bg-gray-100 p-6 rounded-lg border-2 border-gray-300">
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-800 mb-2">{original}</div>
              <div className="text-lg text-gray-600">Simplify to lowest terms</div>
            </div>
          </div>
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="bg-gray-100 p-6 rounded-lg border-2 border-gray-300">
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-800 mb-2">{original}</div>
              <div className="text-lg text-gray-600">Convert this</div>
            </div>
          </div>
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="bg-gray-100 p-6 rounded-lg border-2 border-gray-300">
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-800 mb-2">{fraction}</div>
              <div className="text-lg text-gray-600">Convert to decimal and percent</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <span className="text-white w-16">Decimal:</span>
              <Input
                type="text"
                placeholder="0.X"
            ...
            <div className="flex items-center space-x-2">
              <span className="text-white w-16">Percent:</span>
              <Input
                type="text"
                placeholder="X%"
...
// Ordering Activity Component for 6.NS.1.e
const OrderingActivity = ({ 
  ...
  // Parse the example to extract numbers to order
  const parseOrderingProblem = (example: string) => {
    // Examples like "0.4, 1/2, 0.75 ordered: 0.4, 1/2 (0.5), 0.75."
    ...
    if (match) {
      ...
      return { numbers, correctOrder };
    }
    return null;
  };

  const problemData = parseOrderingProblem(currentExample);
  ...
  const { numbers, correctOrder } = problemData;
  const availableNumbers = numbers.filter(n => !userOrder.includes(n));

  const addToOrder = (number: string) => {
    setUserOrder([...userOrder, number]);
  };
  ...
  const handleSubmit = () => {
    if (userOrder.length !== numbers.length) {
      handleWrongAnswer('', '', "Please order all numbers before submitting.");
      return;
    }
  ...
  return (
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardHeader>
        ...
        <CardDescription className="text-gray-300">
          Order these numbers from least to greatest: {numbers.join(', ')}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          ...
          {/* Available Numbers */}
          <div className="text-center">
            <h4 className="text-sm font-medium text-gray-300 mb-2">Available Numbers:</h4>
            <div className="flex flex-wrap gap-2 justify-center">
              {availableNumbers.map((number, index) => (
                <Button
                  key={index}
                  onClick={() => addToOrder(number)}
                  variant="outline"
          ...
          {/* User's Ordered List */}
          <div className="text-center">
            <h4 className="text-sm font-medium text-gray-300 mb-2">Your Order (least to greatest):</h4>
            <div className="min-h-12 bg-gray-100 p-3 rounded-lg border-2 border-gray-300 flex items-center justify-center">
              {userOrder.length === 0 ? (
                <span className="text-gray-500 italic">Click numbers above to add them here</span>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {userOrder.map((number, index) => (
                    <Button
                      ...
                      variant="destructive"
                      size="sm"
                      className="bg-purple-100 text-purple-800 border-red-300 hover:bg-red-200"
                    >
                      {number} ✕
                    </Button>
                  ))}
          ...
          <div className="flex space-x-2">
            <Button 
              onClick={handleSubmit}
              disabled={userOrder.length !== numbers.length || feedback?.correct}
              className="text-white bg-blue-600 hover:bg-blue-700"
            >
              Submit Order
            </Button>
...
// Interactive Practice Renderer - now uses database examples
const InteractivePracticeRenderer = ({ 
  ...
  const handleActivityComplete = () => {
    onActivityComplete(subLessonIndex);
  };
  ...
  // Determine activity type based on sub-lesson title and examples
  const title = subLesson.title.toLowerCase();
  
  if (title.includes('order') || title.includes('put') && title.includes('in order')) {
    return (
      <OrderingActivity
        examples={examples}
...
// Interactive Grid component for 6.NS.1.c - user clicks/drags to shade squares
...
const PreShadedGridRenderer: React.FC<PreShadedGridProps> = ({
  ...
  const parseExample = (example: string) => {
    ...
    // Handle strip model descriptions like "A strip with about one-fifth shaded is approximately 20%."
    if (example.includes('one-fifth') && percentMatch) {
      return {
        shadedSquares: 20,
        totalSquares: 100,
        expectedPercent: parseFloat(percentMatch[1]),
    ...
    } else if (example.includes('one-third') && percentMatch) {
      return {
        shadedSquares: 33,
        totalSquares: 100,
        expectedPercent: parseFloat(percentMatch[1]),
    ...
    } else if (example.includes('half') && percentMatch) {
      return {
        shadedSquares: 50,
        totalSquares: 100,
        expectedPercent: parseFloat(percentMatch[1]),
  ...
  const handleSubmit = () => {
    ...
    if (Math.abs(userPercent - currentExampleData.expectedPercent) < 0.5) {
  ...
  // Generate grid based on the example data
  const generateGrid = () => {
    const squares = [];
    const totalSquares = currentExampleData.type === 'grid' ? currentExampleData.totalSquares : 100;
    const shadedCount = currentExampleData.type === 'grid' ? currentExampleData.shadedSquares : currentExampleData.shadedSquares;
  ...
  return (
    <Card className="rounded-lg border border-green-600 text-card-foreground shadow-sm mb-4 bg-[#14532d4d] relative">
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          <div className="text-center">
            <div className="text-lg text-gray-300 mb-2">Look at this {currentExampleData.type}</div>
          </div>

          {/* Pre-shaded Grid or Strip */}
          <div className="bg-gray-100 p-4 rounded-lg border-2 border-gray-300">
            {currentExampleData.type === 'strip' ? (
              // Strip model for benchmark percents
              <div className="flex">
                {Array.from({length: 10}, (_, i) => (
                  <div
                    key={i}
                    className={`w-8 h-16 border border-gray-400 ${
                      i < (currentExampleData.shadedSquares / 10) ? 'bg-green-500' : 'bg-white'
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="text-sm text-gray-300 text-center">
            <p>What percentage is shaded?</p>
          </div>
          
          <div className="flex items-center space-x-2">
            <Input
              type="text"
              placeholder="Enter percent (e.g., 80%)"
              value={userAnswer}
              onChange={(e) => setUserAnswer(e.target.value)}
              className="w-48 text-white bg-gray-700 border-gray-600 placeholder-gray-400"
              disabled={feedback?.correct}
          ...
          <div key={subLesson.id} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {isGridStandard && (subLesson.code?.includes('6.NS.1.c') || subLesson.code?.includes('6-U.3')) && subLesson.examples && subLesson.examples.length > 0 ? (
                // Interactive grid activities for 6.NS.1.c - user shades grid using database examples
                <InteractiveGridRenderer
                  subLesson={subLesson}
                  subLessonIndex={subLessonIndex}
                  onRequestHelp={onRequestHelp || (() => {})}
                  onActivityComplete={handleActivityComplete}
                />
              ) : isGridStandard && (subLesson.code?.includes('6.NS.1.a') || subLesson.examples?.some(ex => ex.includes('6-U.1') || ex.includes('6-U.2'))) && subLesson.examples && subLesson.examples.length > 0 ? (
                // Pre-shaded grid activities for 6.NS.1.a - user identifies percentage using database examples
                <PreShadedGridRenderer
                  subLesson={subLesson}
                  subLessonIndex={subLessonIndex}
                  onRequestHelp={onRequestHelp || (() => {})}
                  onActivityComplete={handleActivityComplete}
                ...
                // Hardcoded grid and strip activities for 6.NS.1.a
                <>
                  {subLessonIndex === 0 ? (
                    // Grid activities for first sub-lesson
                    <div className="space-y-4">
                      {/* ALL EXAMPLES MUST COME FROM DATABASE - NO HARDCODED EXAMPLES */}
                      <div className="text-gray-300 text-center p-4">
                        <p>Please select a specific sub-standard like 6.NS.1.c for interactive grid activities.</p>
                      </div>
                    </div>
                  ) : (
                    // ALL EXAMPLES MUST COME FROM DATABASE - NO HARDCODED EXAMPLES
                    <div className="text-gray-300 text-center p-4">
                      <p>Please select a specific sub-standard for interactive activities.</p>
                    </div>
                  )}
                </>
