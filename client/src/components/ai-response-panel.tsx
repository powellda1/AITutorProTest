...
export default function AiResponsePanel({ height, response, onClear, sessionId, onAiResponse, onExpand, onMinimize }: AiResponsePanelProps) {
  const [message, setMessage] = useState("");
  const [showChat, setShowChat] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const queryClient = useQueryClient();
  
  // Check if panel is minimized (only showing header)
  const isMinimized = height <= 140;
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-gray-700 cursor-move">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button
              onClick={() => {
                if (!showChat) {
                  ...
                  onExpand(); // Expand the panel when user clicks "Ask Question"
                } else {
                  // Close chat AND clear formatted response
                  onClear(); // Clear the formatted response
                  onMinimize(); // Minimize the panel when closing chat
                }
            ...
            {response && (
              <button
                onClick={() => {
                  onClear(); // Clear the response
                  onMinimize(); // Minimize the panel
                }}
