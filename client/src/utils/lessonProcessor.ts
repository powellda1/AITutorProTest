...
export interface ProcessedLessonContent {
  ...
  componentType: 'number-line' | 'ordering' | 'comparison' | 'word-problem' | 'exponent' | 'fraction-operation' | 'scaling' | 'perfect-square' | 'grid' | 'decimal-percent' | 'mixed-number-visual';
  mode?: NumberLineMode;
...
export interface LessonAnalysis {
  ...
  componentType: 'number-line' | 'ordering' | 'comparison' | 'word-problem' | 'exponent' | 'fraction-operation' | 'scaling' | 'perfect-square' | 'grid' | 'strip' | 'decimal-percent' | 'text-input' | 'fraction-visual-input' | 'mixed-number-visual' | 'default';
}
...
export function analyzeLessonType(explanation: string, title: string, standardCode?: string, standardDescription?: string): LessonAnalysis {
  ...
  if (titleLower.includes('convert between improper fractions and mixed numbers') ||
    ...
    // Students work with division process and grouping concepts
    ...
    // Mentions "dividing" which suggests grouping/division visual models
    
    // STEP 3: Review description for learning objectives
    console.log('🔍 STEP 3: Standard description calls for "number lines, and concrete and pictorial models"');
    // Standard description explicitly mentions visual models for fraction/mixed number work
    
    // STEP 4: Determine visual component based on mathematical content requirements
    console.log('🔍 STEP 4: Content requires visual division model to show grouping process (11 items grouped by 4s)');
    console.log('✅ EXAMPLES-FIRST: Mixed number conversion requires visual division model component');
...
function processMixedNumberConversion(originalExample: string): ProcessedLessonContent {
  ...
  // Extract original fraction for visual display
  // For mixed-to-improper conversion, we want the improper fraction (after =)
  // For improper-to-mixed conversion, we want the improper fraction (before =)
  let originalFraction = '11/4'; // fallback
  
  // Try to find improper fraction pattern first
  const improperAfterEquals = originalExample.match(/= (\d+\/\d+)/);
  const improperBeforeEquals = originalExample.match(/^(\d+\/\d+)/);
  
  if (improperAfterEquals) {
    // Mixed-to-improper conversion: "4 1/3 = 13/3"
    originalFraction = improperAfterEquals[1];
  } else if (improperBeforeEquals) {
    // Improper-to-mixed conversion: "11/4 = 2 3/4"
    originalFraction = improperBeforeEquals[1];
  } else {
    // Fallback to any fraction found
    originalFraction = originalExample.match(/(\d+\/\d+)/)?.[1] || '11/4';
  }
